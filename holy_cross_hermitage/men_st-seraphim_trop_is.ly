...
\relative c'' {  \set Score.timing = ##f \key f \major \autoBeamOff 
	...
	f8([^\markup{F} g]) a4 g2^\markup{G} a8([ bes]) c4^\markup{F} bes8([ a]) bes([ c]) \once \override TextScript #'extra-offset = #'( -1 . 0 )d2(^\markup{\raise #1 \arrow-head #1 #up ##t B \raise #1 {\flat}} c4) bes2 \Z c4.(^\markup{F} bes8 a[ bes] c4 bes4. a8 g[ a]) bes4 a8([ g]) a([ bes])	c2( bes4) a8([ bes]) c([ a]) bes2^\markup{G} \W
	a2^\markup{F} g4^\markup{ \raise #1 \arrow-head #1 #up ##t Un.} g8([ f]) \Z g([^\markup{E} a g f]) e([ f]) g([ a]) a( g4.) \J 
	g8([^\markup{Un.} f]) g([^\markup{G}  a]) bes4 a8([ bes]) \padNote c4 bes8([ a]) bes([ a]) bes([ a]) g([ f])^\markup{F} g([ a]) \Z bes4( a g8[ a]) \padNote bes4 a2 \once\override Script #'script-priority = #-1  g1^\markup{G} \fermata \bar "|."
	}
\addlyrics { 
